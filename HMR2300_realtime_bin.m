...
function [BX, BY, BZ, time, intervals] = HMR2300_realtime_bin(comPort, duration)
    %% Output variables - memmory preallocation
    Ts=0.050; % Sampling time (configurable)
    iterations = round(duration/Ts);
    ...
    while i<=iterations
        ...
        [BX(i), BY(i), BZ(i)]=HMR2300_binread(sensor); %Unit: Gauss. Obs.: to_microTesla_factor=*100
